<template>
  <a-layout id="components-layout-demo-side" style="min-height: 100vh;background-color:#ffffff ">
    ...
    <a-layout-content :style="{   background: '#FAFBFF', minHeight: '280px', }">
        ...
        <a-row style="color: white">
          <a-col span="12">
            <a-breadcrumb>
              ...
              <a-breadcrumb-item style="color: white">My Teams</a-breadcrumb-item>
      ...
      <div style="height: 80vh;overflow-y: scroll;margin-bottom: 1rem">
              ...
              <div style="color: black">
                ...
                <a-row>
                  ...
                  <a-col span="24">
                    ...
                    <p v-if="edit">Edit Team</p>
                    <p v-else>Create new Team</p>
                  ...
                  <a-col span="24">
                    ...
                    <a-form-item
                        ...
                        :wrapper-col="{ span: 24 }"
                    >
                      <a-input placeholder="Team Name" v-model="organization" v-validate="{required: true}"
                    ...
                    <p style="font-family: sofia_prolight">You can invite members after creation.Just click on the edit of created team</p>
                    <div v-if="edit">
                        ...
                        <div v-else>
                          ...
                          <div v-else>
                            <a-result status="404" sub-title="Sorry, You are not part of a team or an organization.">
                              <template #extra>
                                <p>You can create one and add member on the left or check invitations </p>
                              </template>
                            </a-result>
...
class Team {
  constructor(id, organization, active, members, invitees,is_admin) {
    this.id = id;
      ...
      if (this.user_organizations.length > 0) {
        ...
        this.user_organizations.forEach(team => {
          ...
          team.members.forEach(member => {
            ...
            team.active.forEach(accepted => {
              if (accepted.user.email === member.invitee_identifier) {
  ...
  methods: {
    getOrganization() {
      ...
      Organizations.myorganizations(this.$store.state.user.pk, auth)
          .then(resp => {
            ...
            //   team['members'] = []
            //   team['invitees'] = []
            //   let accepted_members =[]
            //   let invited_members =[]
            //
            //   Organizations.organizationUsers(team.organization.id, auth)
            //       .then(users => {
            //
            //         accepted_members = users.data
            //         accepted_members.forEach(removeadmin => {
            //           if (removeadmin.is_admin) {
            //             const index = accepted_members.indexOf(removeadmin);
            //             if (index > -1) {
            //               accepted_members.splice(index, 1);
            //             }
            //           }
            //         })
            //         team['active'] = accepted_members
            //
            //
            //
            //
            ...
            //           if(invited.data[0].organization.id === team.organization.id){
            //             invited_members = invited.data
            //             team['invitees'] = invited_members
            //
            //
            //             invited_members.forEach(invited=>{
            //               accepted_members.forEach(accepted=>{
            //
            //                 if(accepted.user.email === invited.invitee_identifier){
            //                   const index = invited_members.indexOf(invited);
            //                   if (index > -1) {
    ...
    getOrgMembers(org) {
      if (this.organizations.length > 0) {
        ...
        Organizations.organizationUsers(org.organization.id, auth)
            .then(resp => {
              ...
              this.accepted.forEach(removeadmin => {
                if (removeadmin.is_admin) {
                  const index = this.accepted.indexOf(removeadmin);
                  if (index > -1) {
                    this.accepted.splice(index, 1);
                  }
                }
              })
              this.ComputeInvitedMembers(org)
          ...
          Organizations.InvitationsSent(team.organization.id, auth)
              .then(resp => {
                resp.data.forEach(user=>{
                  ...
                  this.allwaiting.push(user)
                })
                this.getOrgMembers(team)
    ...
    ComputeInvitedMembers(org) {
     this.waiting.forEach(invited=>{
       this.accepted.forEach(active=>{
         if(invited.invitee_identifier === active.user.email && invited.organization.id === active.organization.id){
           const index = this.waiting.indexOf(invited);
           if (index > -1) {
      ...
      let is_admin = org.is_admin
      let active =[]
      let members=[]
      let invitees =[]
      this.accepted.forEach(one_user=>{
        if(one_user.organization.id === org.organization.id){
          active.push(one_user)
        }
      })
      this.waiting.forEach(one_user=>{
        if(one_user.organization.id === org.organization.id){
          members.push(one_user)
        }
      })
      this.allwaiting.forEach(one_user=>{
        if(one_user.organization.id === org.organization.id){
          invitees.push(one_user)
        }
      })
      let team_obj = new Team(
          id, organization, active, members, invitees,is_admin
      );
    ...
    submitForm() {
      ...
      if (this.edit) {
        let current = this.currentOrganization
              ...
              if (this.tags.length > 0) {
                ...
                this.tags.forEach(email => {
                  ...
                  Organizations.createInviteoganization(invitee_obj, auth)
                      .then(() => {
                        Organizations.InviteEmail(current.id,email,auth)
                        this.tags = []
                      })
    ...
    RemoveMember(email) {
      ...
      this.currentOrganization.invitees.forEach(invitee=>{
        if(email.user.email === invitee.invitee_identifier){
          Organizations.RejectInvitation(invitee.id, auth)
