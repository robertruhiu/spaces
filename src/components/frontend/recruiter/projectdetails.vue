                        ...
                        <a-col :span="10" style="padding: 0 1%;">
                            <div style="border:1px solid #e8e8e8;;padding: 3%;">
                                ...
                                <div style="margin-left: 5%" v-if="application.project === null">
                                    <a-button type="primary" @click="AssignProject(project.id,ApplicationId,job.id,project.name)">Assign
...
<script>
    ...
    export default {
        ...
        methods: {
            AssignProject(project, application,job,projectname) {
                const auth = {
                    headers: {Authorization: 'JWT ' + this.$store.state.token}

                };
                Marketplace.pickreject(application, {test_stage: 'invite sent', project: project,projectname:projectname}, auth)
                this.$router.push({
                    name: 'job',
                    params: { jobId: job }
                })
