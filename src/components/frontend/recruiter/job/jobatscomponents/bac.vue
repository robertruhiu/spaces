  ...
  methods: {
    ...
    getOrgMembers() {
      ...
      this.organizations.forEach(organization => {
        Organizations.organizationUsers(organization.organization.id, auth)
            .then(resp => {
              organization['accepted']= resp.data
    ...
    getInvitees() {
      ...
      this.organizations.forEach(organization => {
        ...
        Organizations.InvitationsSent(organization.organization.id, auth)
            .then(resp => {
              organization['members']= resp.data
              organization['invitees']= resp.data
              this.organizations[organization] = organization

              resp.data.forEach(one_invite=>{
                this.invited_members.push(one_invite)
              })
    ...
    ComputeInvitedMembers() {
      ...
      let invited = this.invited_members
      let active_users = this.accepted_members
      ...
      //
      //
      //   invited.forEach(one_member => {
      //
      //
      //
      //     if (one_member.organization.id === organization.organization.id) {
      //       invitees.push(one_member)
      //     }
      //
      //   })
      //
      //
      //
      //   active_users.forEach(removeadmin => {
      //     if (removeadmin.is_admin) {
      //       const index = active_users.indexOf(removeadmin);
      //       if (index > -1) {
      ...
      //             const index = invited.indexOf(member);
      //             if (index > -1) {
      ...
      //   this.organ_list.push(organization)
      //   this.loading = false
      //
      // })
